...
class Encoder(nn.Module):
	def __init__(self, av_size, d_size, gc_size, z_size, rep_size):
		...
		self.gc_2 = GraphConvolution(gc_size, gc_size)
...
class DecoderPart(nn.Module):
	def __init__(self, z_out_size, rep_size):
		...
		self.decode = nn.Sequential(
			...
			nn.ReLU(),
			nn.Linear(int(self.rep_size / 2), int(self.rep_size / 4)),
...
class Decoder(nn.Module):
	def __init__(self, z_out_size, rep_size):
		...
		self.decode = nn.Sequential(
			...
			nn.ReLU())
		'''
		self.decoder_d = DecoderPart(z_out_size, rep_size)
		#self.decoder_t = DecoderPart(z_out_size, rep_size)

	def forward(self, x):
		#rec_xt = self.decoder_t(x)
		rec_xd = self.decoder_d(x)
		#x = cat_mtrx(rec_xt, rec_xd)
		return rec_xd
...
class Discriminator1(nn.Module):
	...
	def __init__(self, d_size, h_size, rep_size, dropout):
		...
		self.normalize1 = nn.InstanceNorm2d(1)
